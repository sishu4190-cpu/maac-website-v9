...
interface Category { id: string; name: string; count: number; desc: string; icon: string; href: string; bg: string; }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 stagger-children">
            {productCategories.map((cat) => (
              <Link key={cat.id} href={cat.href} className="category-card reveal-scale block" style={{ textDecoration: "none" }}>
                ...
                <div style={{
                  background: `linear-gradient(135deg, ${cat.bg} 0%, #0f2d1a 100%)`,
                  minHeight: 220, padding: "2rem",
                  display: "flex", flexDirection: "column", justifyContent: "space-between",
                  position: "relative", borderRadius: 14, overflow: "hidden",
                }}>
                  {/* Subtle molecule pattern */}
                  <div className="molecule-bg" style={{ position: "absolute", inset: 0, opacity: 0.6 }} />
